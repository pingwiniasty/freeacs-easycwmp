...
ENV ACS_URL=http://192.168.26.1:8085/tr069
...
ADD filesystem/easycwmp /opt/dev/easycwmp/ext/openwrt/config/
...
ln -sf bash /bin/sh
RUN cp /opt/git/libubox/libblobmsg_json.so /usr/lib

ADD filesystem/startup.sh /

#RUN sed -i "s|%ACS_URL%|${ACS_URL}|g" /opt/dev/easycwmp/ext/openwrt/config/easycwmp
#RUN cat /opt/dev/easycwmp/ext/openwrt/config/easycwmp
ENTRYPOINT ["/bin/bash"] 
CMD ["/startup.sh"]
